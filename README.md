...
- Session IDs (`<mac>:<counter>`) aren't unique across process restarts —
  the counter resets to 1 each time the script starts. Harmless for
  InfluxDB storage (it's a field, not part of a point's identity), but worth
  knowing if cross-referencing a stored `session_id` back to a specific log
  line long after a restart.
